...
Formally, virtualization can be defined as a mapping between a guest state ($S_i$) and a host state ($S'_i$), such that a sequence of operators, $e_k$, modifying the guest state from ($S_i$) to ($S_j$), can be represented by some corresponding sequence of operators, $e'_k$, which modifies the host state from ($S'_i$) to ($S'_j$) respectively \cite{cit:smith_nair}. 
...
\begin{figure}[H]
  ...
  \caption{Equivalent State Mapping \cite{cit:smith_nair}}
  \label{fig:statemap}
\end{figure}
...
Research and innovation into devices has allowed a new class of memory technologies to spring up under the banner of Storage Class Memory which fall in the NVRAM category. Some common examples are PCM (Phase change memory) \cite{cit:pcm}, STT-RAM (Spin Transfer Torque RAM) \cite{cit:sttram}, ReRAM (Resistive RAM) \cite{cit:reram}, FRAM (Ferroelectric RAM) \cite{cit:feram}, MRAM (Magnetic RAM) \cite{cit:mram}, etc. Each of these devices have different properties in terms of power efficiency, speed, density, and cost/bit, but are unified by the following common characteristics.
...
Introduction of SCM will see a change in the motherboard architecture (Fig~\ref{fig:new_arch}). It can be placed alongside DRAM on the memory bus, and also on a PCIe bus along with SSDs. With the growth and commercialization of different SCM technologies, SCM is expected to replace/augment both DRAM and Flash memory from their dominant roles in conventional machines in the near future. 
...
Popularity and demand for virtualization led Intel to introduce various features for the x86 platform, in VT-x \cite{cit:intel_manual} and VT-d \cite{cit:intelvtd} extensions. These extensions recognize the following additional modes of operation of the CPU.
